    ...
    for (let squareIndex = 0; squareIndex < 9; squareIndex++) {
      ...
      for (let solution = 1; solution < 10; solution++) {
        ...
        cells.forEach((cell) => {
          if (cell.squareIndex === squareIndex) {
            ...
            if (cell.possibleSolutions.includes(solution)) {
              if (
                !columnIndicesContainingSolution
                  .get(solution)!
                  .includes(cell.columnIndex)
              ) {
                columnIndicesContainingSolution
                  .get(solution)!
                  .push(cell.columnIndex);
              }
            }
          }
        });
        ...
        solutionsThatCanAlreadyBeWorkedOut.forEach((s) => {
          columnIndicesContainingSolution.set(s, []);
        });
      }
              ...
              if (cell.squareIndex !== squareIndex) {
                ...
                if (cell.possibleSolutions.includes(solution)) {
                  ...
                  console.log(
                    `Square ${
                      squareIndex + 1
                    } (by column) eliminates ${solution} from row ${
                      cell.rowIndex + 1
